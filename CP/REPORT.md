...
```
## Выводы
В ходе курсовой работы мне далась возможность на более практической задаче применить знания, полученные в течении курса. В первую очередь я узнал о существовании специального формата файлов, хранящих генеологические деревья. Для меня это стало настоящим открытием.

Сам же язык  Prolog оказался невероятно удобным инструментом для анализа древовидных структур. Мне не приходилось писать специальные контсрукции обхода дерева и поиска в нём, язык делает это за меня. От меня требуется только верно выстроить запрос к нему.

Особенно приятно оказалось использовать те конструкции, которые были написаны мною еще в предыдущих лабораторных. Это показывает насколько были полезны эти лабораторные, чтобы подготовить меня к курсовой работе.